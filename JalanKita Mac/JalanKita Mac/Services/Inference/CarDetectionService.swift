...
final class CarDetectionService {
    ...
    /// Resolution order (mirrors InferenceService.resolveWorkerLaunch()'s
    /// dev tiers -- there's no packaged/frozen tier yet, see type doc):
    ///  1. `JALANKITA_V13_REPO` + `JALANKITA_V13_PYTHON` env vars, for anyone
    ///     whose checkout doesn't match the layout below (e.g. a separate
    ///     full v13 dev checkout with more than the trimmed runtime files).
    ///  2. Auto-detected: `CarDetection/` as a sibling of `PythonWorker/`
    ...
    ///     PythonWorker/README's dev setup. Both env vars override
    ///     independently so only the mismatched half needs setting.
