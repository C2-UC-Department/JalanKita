...
struct RoadDamageResponse: Decodable {
    ...
    /// The same sentence `worker_main.py` sends, as a floor for any response that
    /// arrives without one.
    ///
    /// The caveat is a property of the DETECTOR, not of the transport, so it must
    /// survive a worker that forgets to attach it. ⚠️ It used to have a second and
    /// larger job — caveating the Stage 0 CSV path, which had no `warnings` column
    /// at all — and that path is gone with `RoadDamageDataset` (ADR-024). What is
    /// left is purely the fallback.
    ///
    ...
    /// pair that changes together. Put it in a view and it escapes the one rule
    /// that would catch the drift. The live value always wins
    /// (`response.warnings ?? domainGapCaveat`), so editing the Python alone
    /// changes what the reviewer reads immediately; the constant is the floor, not
    /// the source of truth.
