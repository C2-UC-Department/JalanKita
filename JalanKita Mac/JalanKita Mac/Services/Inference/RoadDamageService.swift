...
//  JalanKita Mac
//
//  App-facing entry point for the road-damage pipeline. Owns the lazily-launched,
//  kept-warm RoadDamageWorkerProcess and turns its NDJSON responses into the one
//  thing AppModel wants: `[ReviewFrame]` for a session.
//
//  This is stage 3 of the video pipeline, running after v13 and OFRSNet but
//  depending on neither — it samples its own frames from the same clip, so a clip
//  with zero parked cars still yields road-damage findings.
//
...
final class RoadDamageService {
    ...
    /// 2,7 s — ~1,7 s detector plus ~1,0 s extent model. Frame count is exactly
    /// `duration / interval`, so the stage costs `duration × 2,7 / interval`: at a
    /// 3 s interval that is 0,9× the clip's own duration, at the 5 s default 0,54×,
    /// and at 1 s nearly three times the clip. That measurement is why the default
    /// is 5 s rather than the 3 s the extractor itself defaults to.
    ///
    /// `clipOffset` shifts every frame's timestamp onto the session axis — 0 for a
    /// single-clip manual upload, the `clipOffsets` running total for clip N of a
    /// synced session.
    ///
    ...
    func analyzeVideo(videoURL: URL,
        ...
        // on their footage, and it is the only place those counts exist.
        onManifest?(manifest)
    ...
    /// One level further out than the repo root — the sibling
    /// `test-road-damage-detection` checkout, whose `.venv` is the last-resort
    /// interpreter when `RoadDamage/` has none of its own.
    ...
    /// Builds one queue-ready `ReviewFrame` from a worker response.
    ///
    /// `clipOffset` is where this frame's own clip starts within the session, in
    /// seconds — 0 for a single-clip manual upload, the running total from
    /// `clipOffsets` for clip N of a synced session. Added to the frame's
    /// clip-relative timestamp it yields `sessionRelativeSeconds`, which is what
    /// puts a damage tick on the same axis as a parking one.
    ///
    ...
    private static func makeFrame(from response: RoadDamageResponse,
        ...
        // Clip-relative first, then shifted onto the session's axis. nil for a photo,
        // which has no timeline to sit on.
        let sessionSeconds = RoadDamageFrameNaming.clipRelativeSeconds(fromStem: stem)
            .map { $0 + clipOffset }

        return ReviewFrame(
            id: stem,
            // A frame sampled from a clip carries its own provenance in its name —
            // `IMG_0040_f000030_t0001000` — so the parsers fill these in for free.
            // An ad-hoc photo matches none of that, and falls back to the filename
            // and em-dashes rather than inventing a clip and a timestamp it does
            // not have.
            sourceClip: RoadDamageFrameNaming.looksSampled(stem)
                ? RoadDamageFrameNaming.sourceClip(fromStem: stem) : imageURL.lastPathComponent,
            frameNumber: RoadDamageFrameNaming.frameIndex(fromStem: stem),
            // Show the session-relative time when there is one, so the label under a
            // frame matches the position of its tick on the timeline. Falls back to
            // the stem's own clip-relative value.
            timecode: RoadDamageFrameNaming.timecode(seconds: sessionSeconds, stem: stem),
            capturedAt: nil,
